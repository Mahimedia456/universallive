-- Universal Live Backend Phase 10 — REPAIRED
-- Creator assets + cloud presets/templates metadata.
--
-- Fix for PostgreSQL ERROR 42P10:
-- ON CONFLICT (preset_key) could not infer the previous partial unique index.
-- A normal UNIQUE index is safe here because PostgreSQL permits multiple NULLs.
...
-- Remove the old partial index if it exists.
drop index if exists public.ul_scene_presets_key_uidx;

-- Normal UNIQUE index allows ON CONFLICT (preset_key).
-- PostgreSQL UNIQUE indexes still allow multiple NULL preset_key values.
...
(
    'gaming-basic',
    'Gaming',
    'Gameplay-first broadcast scene.',
    'gaming',
    true,
    true,
...
select preset_key, name, category, is_system, is_active
from public.ul_scene_presets
order by preset_key;
