export type AppEnv = {
  ...
  STREAM_SECRET_MASTER_KEY: string;
...
function required(raw: unknown, name: string): string {
...
export function validateEnv(raw: Record<string, unknown>): AppEnv {
  ...
  const supabaseUrl = required(raw.SUPABASE_URL, 'SUPABASE_URL');
  if (!/^https:\/\/[a-z0-9-]+\.supabase\.co$/i.test(supabaseUrl)) {
    throw new Error('SUPABASE_URL is invalid');
  }

  const authMode = String(raw.AUTH_MODE ?? 'universallive-db');
  if (authMode !== 'universallive-db') {
    throw new Error('AUTH_MODE must be universallive-db');
  }

  const emailMode = String(raw.EMAIL_MODE ?? 'console') as AppEnv['EMAIL_MODE'];
  if (!['smtp', 'console'].includes(emailMode)) {
    throw new Error('EMAIL_MODE must be smtp or console');
  }
  if (nodeEnv === 'production' && emailMode !== 'smtp') {
    throw new Error('EMAIL_MODE=smtp is required in production');
  }
  ...
  if (emailMode === 'smtp' && (!smtpHost || !smtpUser || !smtpPass)) {
  ...
  return {
    ...
    OTP_RESEND_SECONDS: positiveInteger(raw.OTP_RESEND_SECONDS, 45, 'OTP_RESEND_SECONDS'),
    ...
    SMTP_USER: smtpUser,
    SMTP_PASS: smtpPass,
    SMTP_FROM_EMAIL: String(raw.SMTP_FROM_EMAIL ?? 'no-reply@universallive.app').trim(),
    SMTP_FROM_NAME: String(raw.SMTP_FROM_NAME ?? 'Universal Live').trim(),
    STREAM_SECRET_MASTER_KEY: secret(raw.STREAM_SECRET_MASTER_KEY, 'STREAM_SECRET_MASTER_KEY'),
    ADMIN_API_KEY: String(raw.ADMIN_API_KEY ?? '').trim() || undefined,
    FIREBASE_PROJECT_ID: String(raw.FIREBASE_PROJECT_ID ?? '').trim() || undefined,
    FIREBASE_CLIENT_EMAIL: String(raw.FIREBASE_CLIENT_EMAIL ?? '').trim() || undefined,
    FIREBASE_PRIVATE_KEY: String(raw.FIREBASE_PRIVATE_KEY ?? '').trim() || undefined,
    GOOGLE_PLAY_PACKAGE_NAME: String(raw.GOOGLE_PLAY_PACKAGE_NAME ?? '').trim() || undefined,
  };
}
